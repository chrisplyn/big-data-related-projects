...
object Metrics {
  ...
  def purity(clusterAssignmentAndLabel: RDD[(Int, Int)]): Double = {
     ...
     */
    val N = clusterAssignmentAndLabel.count().doubleValue()

    clusterAssignmentAndLabel.groupByKey.map {
      case (trueClass, valueList) =>
        valueList.toList.groupBy(identity).mapValues(_.size).valuesIterator.max
    }.sum() / N
  }
}
