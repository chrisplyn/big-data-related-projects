...
object RandomWalk {
  ...
  def randomWalkOneVsAll(graph: Graph[VertexProperty, EdgeProperty], patientID: Long, numIter: Int = 100, alpha: Double = 0.15): List[Long] = {
    ...
    val patientVertex = graph.subgraph(vpred = (id, attr) => attr.isInstanceOf[PatientProperty]).vertices.map(x=>x._1).collect().toSet
    val top_ten = rankGraph.vertices.filter(x => patientVertex.contains(x._1)).takeOrdered(11)(Ordering[Double].reverse.on(x => x._2)).map(_._1)
